...
plot_cells_alt <-
  function (cds,
    ...
    p <- ggplot() +
      ...
      labs(x = "UMAP 1", y = "UMAP 2", color = "Log10(Expr)") +
...
custom_variable_plot <- function(cds,
                                 ...
                                 w = NULL,
                                 palette_viridis = T,
                                 optional_levels = NULL,#use to relevel categorical variable
                                 facet_row = NULL,
                                 facet_col = NULL
) {
  if (!is.null(i)) {
  ...
  if (!is.null(optional_levels)) {
    data_long$value<-factor(data_long$value, levels = optional_levels)
  }
  plot<-ggplot()
  ...
  if(class(data_long$value) %in% c("numeric","integer")) {
    plot<-plot+scale_fill_viridis_c(guide = F)+scale_color_viridis_c(guide = F)
  } else if (palette_viridis == T) {
    plot<-plot+
      scale_fill_viridis_d(begin = 0.2,end = 0.8)+
      scale_color_viridis_d(begin = 0.2,end = 0.8, guide = F)
  } else {
    colourCount = length(unique(data_long$value))
    getPalette = colorRampPalette(brewer.pal(9, "Set1"))
    plot<-plot+
      scale_color_manual(guide = F,values = getPalette(colourCount))+
      scale_fill_manual(values = getPalette(colourCount))
  }
  if(class(data_long$value) %in% c("numeric","integer")){
    plot<-plot+guides(fill = guide_colorbar(override.aes = list(alpha = 1)))
  } else {
  ...
  if (!is.null(facet_row) && is.null(facet_col)) {
    plot<-plot+facet_wrap(facets = vars(!!sym(facet_row)))+
      theme(strip.background = element_blank())
  }
  if (!is.null(facet_row) && !is.null(facet_col)) {
    plot<-plot+facet_grid(rows = vars(!!sym(facet_row)), cols = vars(!!sym(facet_col)))+
      theme(strip.background = element_blank())
  }
  plot<-plot+theme(legend.position = legend_pos)#+coord_fixed()
  plot<-plot+scale_x_continuous(breaks = pretty_breaks(n = 3))+scale_y_continuous(breaks = pretty_breaks(n = 3))
...
custom_violin_plot <-
  function(cds,
    ...
    if (!is.null(xangle)) {
      p1 <- p1 + theme(axis.text.x = element_text(angle = xangle, vjust = 0.5))
    }
    if (!is.null(outfile)) {
